B_BAN_run <- function(J, nperiod, nobv, b, truep){
  ...
  for (i in 1:J){
    tr1 = sample(c(0,1),1)  #decides which treatment to choose randomly
    tr2 = 1-tr1   #ensures the other treatment is chosen next
    Trt = c(tr1,tr2)
  ...
  for (numpd in 3:nperiod){
    for (pid in 1:J){
      ...
      wA = (wA)^c/(wA^c + wB^c) 
      wB = 1- wA
      allocprob[numpd-2,pid] = wA
      
      t.next = sample(c(0,1), size=1, prob=c(wA,wB))  #treatment chosen based on allocations probs
      #if (wA > wB){t.next = 0} else {t.next = 1} #treatment chosen deterministically
